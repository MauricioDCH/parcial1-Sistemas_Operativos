...
int main(int argc, char* argv[]) {
    ...
    else if (argc == 2)
    {
        ...
        if(std::string(argv[1]) == "--encrypt" || std::string(argv[1]) == "-e") {
            ...
            if (vector_fd_original.datos.empty()) {
                write(STDERR_FILENO, "No se pudo leer el archivo.\n", 28);
            } else {
                write(STDOUT_FILENO, "Archivo leído correctamente.\n", 30);
            }
            ...
            int file_descriptor_salida_encriptado = escribirArchivo.escribirArchivo(vector_fd_original_encriptado.fileDescriptor_Encriptado,
            ...
            int file_descriptor_salida_desencriptado = escribirArchivo.escribirArchivo(vector_fd_encriptado_desencriptado.fileDescriptor_Desencriptado,
                vector_fd_encriptado_desencriptado.datos_Desencriptado,
                nombre_archivo_desencriptado);

            // ---------------------------------------------------------------------------------------------------------------------------------
            // SECCIÓN PARA CERRAR EL(LOS) ARCHIVOS.
        ...
        /*
        else if (std::string(argv[1]) == "--compress" || std::string(argv[1]) == "-c") {
            write(STDOUT_FILENO, "Comprimir\n", 10);
        }
        else if (std::string(argv[1]) == "--decompress" || std::string(argv[1]) == "-x") {
            write(STDOUT_FILENO, "Descomprimir\n", 13);
        }
        else {
            write(STDOUT_FILENO, buffer_op_invalida, length_formateo_op_invalida);
        }
